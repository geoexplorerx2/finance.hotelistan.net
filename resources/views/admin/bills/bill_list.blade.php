        ...
        <div class="col-md-12 table-responsive">
            <nav aria-label="breadcrumb" class="mt-3">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Senetler</li>
                </ol>
            </nav>
						...
						<div class="subheader py-2 py-lg-4 subheader-transparent" id="kt_subheader">
							<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
								<div class="d-flex align-items-center flex-wrap mr-2">
									<h3 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Senetler</h3>
								</div>
								<div class="d-flex align-items-center">
                                    @can('senetleri ekle')
                                    <button data-toggle="modal" data-target="#billModal" class="btn btn-primary float-right"><i class="fa fa-plus" aria-hidden="true"></i> Yeni Senet Ekle</button>
                                    @endcan
								</div>
							</div>
						</div>
                                ...
                                <tbody>
                                    @foreach ( $bills as $bill )
                                        <tr role="row" class="odd">
                                            ...
                                            <td>{{ $bill->expiry_date }}</td>
                                            <td>{{ $bill->issue_date }}</td>
                                            <td>{{ $bill->amount }}</td>
                                            <td>{{ $bill->companies->name }}</td>
                                            <td>{{ $bill->note }}</td>
                                            <td>
                                                <div class="d-flex justify-content-center flex-shrink-0">
                                                    @can('senetleri duzenle')
                                                    <a href="{{ route('bills.edit', ['id' => $bill->id]) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 inline-popups" title="Güncelle">
                                                    ...
                                                    @endcan
                                                    @can('senetleri sil')
                                                    <a href="{{ route('bills.destroy', ['id' => $bill->id]) }}" onclick="return confirm('Silmek İstediğinize Emin misiniz?');" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" title="Sil">
                                                        <span class="svg-icon svg-icon-3">
                                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Yeni Senet Ekle</h5>
