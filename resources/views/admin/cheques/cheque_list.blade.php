        ...
        <div class="col-md-12 table-responsive">
            <nav aria-label="breadcrumb" class="mt-3">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Çekler</li>
                </ol>
            </nav>
						...
						<div class="subheader py-2 py-lg-4 subheader-transparent" id="kt_subheader">
							<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
								<div class="d-flex align-items-center flex-wrap mr-2">
									<h3 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Çekler</h3>
								</div>
                                ...
                                <tbody>
                                    @foreach ( $cheques as $cheque )
                                        <tr role="row" class="odd">
                                            <td style="background-color:{{ $cheque->status->color }};color:white">{{ $cheque->status->name }}</td>
                                            <td>{{ $cheque->bank_name }}</td>
                                            <td>{{ $cheque->expiry_date }}</td>
                                            <td>{{ $cheque->issue_date }}</td>
                                            <td>{{ number_format($cheque->amount, 2)}}</td>
                                            <td>{{ $cheque->currency }}</td>
                                            <td>{{ $cheque->companies->name }}</td>
                                            <td>{{ $cheque->cheque_no }}</td>
                                            <td>{{ $cheque->note }}</td>
                                            <td>
                                                <div class="d-flex justify-content-center flex-shrink-0">
                                                    @can('cekleri duzenle')
                                                    <a href="{{ route('cheques.edit', ['id' => $cheque->id]) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 inline-popups" title="Güncelle">
                                                    ...
                                                    @endcan
                                                    @can('cekleri sil')
                                                    <a href="{{ route('cheques.destroy', ['id' => $cheque->id]) }}" onclick="return confirm('Silmek İstediğinize Emin misiniz?');" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" title="Sil">
                                                        <span class="svg-icon svg-icon-3">
                                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="modal-body">
                <form action="{{ route('cheques.store') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="name" name="bankName" placeholder="Banka Adı">
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label for="company_id">Firma Adı</label>
